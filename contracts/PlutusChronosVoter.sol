...
contract PlutusChronosVoter is Initializable, OwnableUpgradeable, UUPSUpgradeable {
  ...
  IVoter_V2 private constant VOTER_V2 = IVoter_V2(0xCC5382FE1ecADBF18629C87b9C89a27B438889D4);
  ...
  function voteMax(address[] calldata _poolVote, uint256[] calldata _weights) external onlyHandler {
    ...
    veCHR.increase_unlock_time(10189, MAXTIME);
    veCHR.increase_unlock_time(10197, MAXTIME);
    veCHR.increase_unlock_time(10301, MAXTIME);
    VOTER_V2.vote(TID, _poolVote, _weights);
    VOTER_V2.vote(10189, _poolVote, _weights);
    VOTER_V2.vote(10197, _poolVote, _weights);
    VOTER_V2.vote(10301, _poolVote, _weights);
  }
