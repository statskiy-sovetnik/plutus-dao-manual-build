  ...
  */
  function rewardPerShare(uint256 _rewardRatePerSecond) public view returns (uint128) {
    ...
    unchecked {
      return
        uint128(
          ((block.timestamp - lastRewardSecond) * _rewardRatePerSecond * MUL_CONSTANT) / shares
        );
    }
  }
  ...
  function _deposit(address _from, address _user, uint128 _amount) private {
    ...
    user.plsRewardDebt =
      user.plsRewardDebt +
      int128(uint128(_calculateRewardDebt(accPlsPerShare, _amount)));

    stakingToken.transferFrom(_from, address(this), _amount);

    unchecked {
      if (_prev + _amount != stakingToken.balanceOf(address(this)))
        revert FAILED('invariant violation');
    }
